...
fn impl_next_for_ast(ast: &DeriveInput) -> TokenStream {
    match &ast.data {
        Enum(data) => impl_next_for_enum(ast, data),
...
fn impl_next_for_enum(ast: &DeriveInput, enum_data: &DataEnum) -> TokenStream {
    let mut variant_iterator = enum_data
        .variants
        .iter()
...
fn impl_next_for_enum_variant(
    enum_ident: &Ident,
    current_variant: &Variant,
    next_variant: Option<&Variant>,
) -> TokenStream {
    ...
    match &current_variant.fields {
        ...
        Fields::Unnamed(FieldsUnnamed { unnamed, .. }) => {
            ...
            let result = iter_fields_unnamed(enum_ident, ident, &field_names_pre, next_min);
            quote! {
...
fn iter_fields_unnamed(
    ...
    let iter_copy = fields.iter().rev();

    for &current_field in iter_copy {
        let reset_value = fields
            .iter()
            .take_while(|&&a| format!("{}", a) != format!("{}", current_field));
        let reset_value2 = reset_value.clone();
        let keep_value = fields
            .iter()
            .skip_while(|a| format!("{}", a) != format!("{}", current_field))
            .skip(1);
